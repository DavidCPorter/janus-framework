...
def main(p, t, d, rf, q, l, shards, solrnum, loadnodes, instances=None):
    ...
    try:
        os.makedirs(
            '/Users/dporter/projects/solrcloud/tests_v1/profiling_data/exp_results/rf' + rf + '_s' + shards + '__clustersize' + solrnum)
    ...
    fp = open(
        '/Users/dporter/projects/solrcloud/tests_v1/profiling_data/exp_results/rf' + rf + '_s' + shards + '__clustersize' + solrnum + '/rf' + rf + '_s' + shards + '__clustersize' + solrnum + "::query=" + q + "::proc=" + p + "::threads=" + t + "::dur=" + d + ":::TIME->" + datetime.today().strftime(
            '%Y-%m-%d-%H:%M:%S'), 'w+')
    # for charting
